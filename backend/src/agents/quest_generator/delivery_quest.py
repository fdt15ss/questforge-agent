...
def build_delivery_quest_graph() -> CompiledStateGraph:
    ...
    def build_prompt(state: DeliveryQuestGraphState) -> DeliveryQuestGraphState:
        ...
        return {
            "prompt": (
                "팩토리 스페이스 납품 퀘스트 생성 에이전트입니다.\n"
                f"정규화된 납품 목표: {state['objective']}\n"
                f"원본 요청 payload: {payload}\n"
                "반드시 다음 JSON 스키마 형식의 JSON 객체 하나만 출력하세요. "
                "마크다운 코드 펜스(```json)나 부가 설명은 넣지 마세요.\n"
                "퀘스트 제목과 목표 문장은 한글로 작성하세요.\n"
                "목표 문장에는 납품할 아이템, 수량, 납품 장소가 드러나야 합니다.\n"
                "{\n"
                '  "quest": {\n'
                '    "type": "delivery",\n'
                '    "title": "퀘스트 제목",\n'
