...
DELIVERY_QUANTITY_RULES = {
    "surprise": {"ratio": 0.05, "minimum": 1},
    "daily": {"ratio": 0.10, "minimum": 2},
    "weekly": {"ratio": 0.25, "minimum": 5},
}
...
def _inventory_resource_quantities(payload: dict[str, Any]) -> dict[str, int]:
    """요청 payload에서 납품 후보로 쓸 인벤토리 자원과 보유량을 꺼냅니다.

    Quest Lab은 주로 `game_state.inventory`에 현재 보유 자원을 담아 보냅니다.
    예전 요청 형식인 top-level `resources`도 같이 지원해 수동 요청을 깨지 않게 합니다.
    """
    game_state = payload.get("game_state")
    inventory = game_state.get("inventory") if isinstance(game_state, dict) else None
    if not isinstance(inventory, dict):
        inventory = payload.get("resources")
    if not isinstance(inventory, dict):
        return {}
    ...
    for key, value in inventory.items():
        if (
            ...
            resource_quantities[key.strip()] = int(value)
    return resource_quantities


def _delivery_item_candidates(payload: dict[str, Any], default_item: str) -> list[str]:
    """납품 퀘스트가 사용할 item id 후보 목록을 정합니다.

    사용자가 `item`을 직접 지정하면 그 값을 가장 우선합니다.
    지정하지 않았을 때만 인벤토리의 자원 id를 순서대로 후보로 사용합니다.
    """
    explicit_item = payload.get("item")
    if isinstance(explicit_item, str) and explicit_item.strip():
        return [explicit_item.strip()]

    inventory_candidates = list(_inventory_resource_quantities(payload))
    return inventory_candidates or [default_item]
...
def _delivery_quantity(
    *,
    ...
    """보유량과 퀘스트 타입을 기준으로 납품 수량을 계산합니다.

    인벤토리 기반 퀘스트는 5, 6, 7처럼 단순 증가시키지 않습니다.
    일일/주간/돌발 타입별 비율을 적용하고, 실제 보유량을 넘지 않게 제한합니다.
    """
    available_quantity = inventory_quantities.get(item)
    if available_quantity is None:
        return base_quantity + index

    rule = DELIVERY_QUANTITY_RULES.get(quest_type, DELIVERY_QUANTITY_RULES["daily"])
    ratio_quantity = math.ceil(available_quantity * rule["ratio"])
    requested_quantity = max(rule["minimum"], ratio_quantity)
    return max(1, min(requested_quantity, available_quantity))
...
def _build_delivery_payload(state: DeliveryQuestGraphState) -> dict[str, Any]:
    ...
    item_candidates = state.get("item_candidates") or [state["item"]]
    inventory_quantities = state.get("inventory_quantities", {})
    ...
    for index in range(state["quest_count"]):
        ...
        item = item_candidates[index % len(item_candidates)]
        quantity = _delivery_quantity(
            item=item,
...
def build_delivery_quest_graph() -> CompiledStateGraph:
    ...
    def normalize_payload(
        ...
        payload = state.get("payload", {})
        default_item = "resource_iron_ingot"
        inventory_quantities = _inventory_resource_quantities(payload)
        item_candidates = _delivery_item_candidates(payload, default_item)
        return {
